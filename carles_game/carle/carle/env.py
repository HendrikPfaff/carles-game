...
class CARLE(nn.Module):
    ...
    def set_neighborhood(self):
        ...
        moore_kernel = torch.tensor([[1., 1., 1.], [1., 0., 1.], [1., 1., 1.]], requires_grad=False)
        ...
        self.neighborhood = nn.Conv2d(1, 1, 3, padding=1, padding_mode=my_mode, bias=False)
        ...
        # Why is this loop executed a second time?
        for param in self.neighborhood.parameters():
            param.requires_grad = False

    def set_action_padding(self):

        asymmetry_width = (self.width - self.action_width) % 2
        asymmetry_height = (self.height - self.action_height) % 2
        ...
        self.action_padding = nn.ZeroPad2d(padding=(height_padding, height_padding + asymmetry_height, width_padding, width_padding + asymmetry_width))
